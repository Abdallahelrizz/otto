...
export async function linearApi({ config, input, workspaceId }) {
  ...
  switch (op) {
    case 'list_issues': {
      ...
      const result = await linearGraphQL(token, `query($first:Int){issues(first:$first){nodes{id title state{name} assignee{name} priority createdAt}}}`, { first: limit });
      return result.data?.issues?.nodes ?? result;
    }
    case 'get_issue': {
      const issueId = config.issueId ?? input.id;
      const result = await linearGraphQL(token, `query($id:String!){issue(id:$id){id title description state{name} assignee{name} priority url}}`, { id: issueId });
    ...
    case 'create_issue': {
      const result = await linearGraphQL(token,
        ...
        { title: config.issueTitle ?? input.title, description: config.description ?? input.description, teamId: config.teamId ?? input.teamId }
      );
    ...
    case 'update_issue': {
      const result = await linearGraphQL(token,
        ...
        { id: config.issueId ?? input.id, title: config.issueTitle, stateId: config.stateId }
      );
      return result.data?.issueUpdate ?? result;
    }
    case 'list_teams': {
      const result = await linearGraphQL(token, `{teams{nodes{id name key}}}`);
