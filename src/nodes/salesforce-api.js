...
export async function salesforceApi({ config, input, workspaceId }) {
  ...
  switch (op) {
    case 'query': {
      ...
      return safeRequestJson(`${base}/query?q=${encodeURIComponent(soql)}`, { headers });
    }
    case 'get_record': {
      const sobject = config.sobject ?? 'Account';
      const recordId = config.recordId ?? input.id;
      return safeRequestJson(`${base}/sobjects/${sobject}/${recordId}`, { headers });
    }
    case 'create_record': {
      const sobject = config.sobject ?? 'Account';
      const fields = config.fields ? JSON.parse(config.fields) : input;
      return safeRequestJson(`${base}/sobjects/${sobject}`, { method: 'POST', headers, body: JSON.stringify(fields) });
    }
    case 'update_record': {
      const sobject = config.sobject ?? 'Account';
      const recordId = config.recordId ?? input.id;
      const fields = config.fields ? JSON.parse(config.fields) : input;
      await safeRequestJson(`${base}/sobjects/${sobject}/${recordId}`, { method: 'PATCH', headers, body: JSON.stringify(fields) });
      return { updated: true, id: recordId };
    }
    case 'delete_record': {
      const sobject = config.sobject ?? 'Account';
      const recordId = config.recordId ?? input.id;
      await safeRequestJson(`${base}/sobjects/${sobject}/${recordId}`, { method: 'DELETE', headers });
